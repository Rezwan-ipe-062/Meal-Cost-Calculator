...
export default function BalanceDashboard() {
  ...
  return (
    <div className="p-4 pb-24 min-h-screen" style={{ background: 'var(--mc-black)' }}>
        ...
        {MEMBERS.map((m, i) => {
          ...
          return (
            <motion.div key={m.id} initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: i * 0.1 }} className="pixel-border p-4" style={{ background: 'var(--mc-brown)' }}>
              ...
              {balance >= 0 ? (
                <p style={{ color: 'var(--mc-green)' }}>
                  🟢 Will Receive ৳{balance.toFixed(2)}
                </p>
              ) : (
                <p style={{ color: 'var(--mc-red)' }}>
      ...
      {!showSettle ? (
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setShowSettle(true)} className="pixel-btn w-full py-3 text-lg font-bold mb-4" style={{ background: 'var(--mc-gold)', color: 'var(--mc-black)' }}>
          Clear Settlement
        </motion.button>
      ) : (
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="pixel-border p-4 mb-4" style={{ background: 'var(--mc-brown)' }}>
          <h3 className="text-lg font-bold mb-3">Record Payment</h3>
          <select value={settleFrom} onChange={e => setSettleFrom(e.target.value)} className="pixel-input w-full p-3 mb-2">
            <option value="">From...</option>
            {MEMBERS.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
          </select>
          <select value={settleTo} onChange={e => setSettleTo(e.target.value)} className="pixel-input w-full p-3 mb-2">
            <option value="">To...</option>
            {MEMBERS.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
          </select>
          <input type="number" value={settleAmount} onChange={e => setSettleAmount(e.target.value)} className="pixel-input w-full p-3 mb-3" placeholder="Amount ৳" />
          <div className="flex gap-2">
            <button onClick={() => setShowSettle(false)} className="pixel-btn flex-1 py-3" style={{ background: 'var(--mc-stone)', color: 'white' }}>Cancel</button>
            <button onClick={handleSettlement} className="pixel-btn flex-1 py-3 font-bold" style={{ background: 'var(--mc-grass)', color: 'white' }}>Confirm</button>
          </div>
        </motion.div>
      )}
      ...
      <div className="space-y-2">
        {expenses.slice(0, 10).map(e => (
          <div key={e.id} className="pixel-border-sm p-2 text-sm" style={{ background: '#2a2a2a' }}>
        ...
        {expenses.length === 0 && <p className="text-gray-500 text-center">No expenses yet</p>}
      </div>
    </div>
  )
}
